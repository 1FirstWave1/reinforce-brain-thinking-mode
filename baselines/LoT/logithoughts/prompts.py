...
SYS_PROMPTS = {
    ...
    'GPQA':{0 :base_sys_prompt},
    'TruthfulQA':{0 :base_sys_prompt},
}
...
ANSWER_PROMPTS = EasyDict(
    {
        ...
        "GPQA":"Next, a single-choice question response will be provided. Please return only one option indicator from the list [OptA, OptB, OptC, OptD]",
        "GSM8K": "Therefore, the numerical (int or float) result is: ",
        "TruthfulQA":"Therefore, the final answer is (chose only one option indicator from the list [OptA, OptB, OptC, OptD, OptE, OptF]:",
